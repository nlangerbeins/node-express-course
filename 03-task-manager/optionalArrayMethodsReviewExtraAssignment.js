...
// Create a new array with only each person's last name
const onlyLastName = names.map((i) => {
  return i.split(' ').pop();
});
console.log('last name:', onlyLastName);

// Filter names that don't match the format "<first> <last>"
const filterNames = names.filter((name) => {
  const parts = name.split(' ');
  console.log(parts);
  return parts.length === 2 && parts.every((el) => el !== '') ? name : null;
});
...
const rightFormatRegex = /^\w+ \w+$/;

// Last Challenge:
const resultArray = names
  .filter((name) => name.match(rightFormatRegex))
  .map((name) =>
    name
      .toLowerCase()
      .split(' ')
  ...
  .filter((person) => !person.endsWith('z'))
  .map((name) => `${name}, please sign up`);
